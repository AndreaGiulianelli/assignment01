    ...
    begin
        ...
        SimulationCicle:
            while currentIteration < simulation.iterations do
                ...
                OkPos:
                  await \A w \in Workers: posBarrier[w] = TRUE; 
                  posBarrier := [w \in Workers |-> FALSE];
            end while;
        Process:
...
fair+ process worker \in Workers
    ...
    begin MainLoop:
        while simulation.completed = FALSE do
            ...
            NotifyMaster:
                skip;
            posBarrier[self] := TRUE;
            AwaitPosConsistency:
                await posBarrier[self] = FALSE;
        end while;
end process;
...
\* BEGIN TRANSLATION (chksum(pcal) = "89ca9976" /\ chksum(tla) = "489112cf")
...
SimulationCicle == /\ pc[Master] = "SimulationCicle"
                   /\ IF currentIteration < simulation.iterations
                         THEN /\ pc' = [pc EXCEPT ![Master] = "AwaitComputation"]
...
ProcessLocal == /\ pc[Master] = "ProcessLocal"
                ...
                /\ UNCHANGED << simulation, iterationsDone, bodies, jobs, 
                                nWorkers, posBarrier, bodyIndex >>

OkPos == /\ pc[Master] = "OkPos"
         /\ \A w \in Workers: posBarrier[w] = TRUE
         /\ posBarrier' = [w \in Workers |-> FALSE]
         /\ pc' = [pc EXCEPT ![Master] = "SimulationCicle"]
...
master == ScheduleWork \/ SimulationCicle \/ AwaitComputation
             \/ ProcessLocal \/ OkPos \/ Process
...
NotifyMaster(self) == /\ pc[self] = "NotifyMaster"
                      /\ TRUE
                      /\ posBarrier' = [posBarrier EXCEPT ![self] = TRUE]
                      /\ pc' = [pc EXCEPT ![self] = "AwaitPosConsistency"]
                      /\ UNCHANGED << simulation, iterationsDone, bodies, jobs, 
                                      nWorkers, currentIteration, bodyIndex >>

AwaitPosConsistency(self) == /\ pc[self] = "AwaitPosConsistency"
                             /\ posBarrier[self] = FALSE
...
worker(self) == MainLoop(self) \/ CalculateForceAndAcceleration(self)
...
\* Last modified Tue Apr 05 23:48:24 CEST 2022 by andrea
